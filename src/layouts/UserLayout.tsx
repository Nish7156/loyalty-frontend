...
export function UserLayout() {
  ...
  // Show theme toggle on scroll only
  useEffect(() => {
    const handleScroll = () => {
      const scrolled = window.scrollY > 50;
      setShowThemeToggle(scrolled);
    };
  ...
  return (
    <div className="user-theme flex flex-col min-h-screen min-h-[100dvh] text-[var(--user-text)] safe-area" data-theme={resolvedTheme} style={{ background: 'var(--user-bg)' }}>
      <header className="fixed top-0 left-0 right-0 z-30 safe-area-top safe-area-x backdrop-blur-md" style={{ padding: '18px 20px 14px', backgroundColor: 'var(--user-nav-bg)', borderBottom: '1px solid var(--user-nav-border)' }}>
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden safe-area-x" style={{ paddingTop: 'calc(70px + env(safe-area-inset-top, 0px))', paddingLeft: '18px', paddingRight: '18px', paddingBottom: 'calc(90px + env(safe-area-inset-bottom, 0px))' }}>
        <Outlet />
      </main>
      {feedbackOpen && (
      ...
      {/* Fixed Theme Toggle Button - shows only on scroll, above bottom bar */}
      {showThemeToggle && (
        <button
