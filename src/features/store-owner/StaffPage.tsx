  ...
  return (
    <div className="min-w-0 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6" style={{ background: 'var(--bg)' }}>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2" style={{ color: 'var(--t)' }}>Staff Management</h1>
          <p style={{ color: 'var(--t2)' }}>Manage your team members and their access</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className="mb-8 rounded-2xl p-6" style={{ background: 'var(--s)', border: '1px solid var(--bdl)', boxShadow: '0 1px 3px rgba(26,24,22,0.05)' }}>
          <h2 className="text-xl font-semibold mb-6" style={{ color: 'var(--t)' }}>Add New Staff Member</h2>
          <form onSubmit={handleCreate} className="max-w-2xl">
            ...
            <p className="text-xs mt-2 mb-6 flex items-center gap-1.5" style={{ color: 'var(--t3)' }}>
              <span className="material-symbols-rounded" style={{ color: 'var(--a)', fontSize: '16px' }}>lightbulb</span>
              Staff will log in using this phone number via OTP
            </p>

            <div className="mb-6">
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--t)' }}>Assign to Branch</label>
              <select
                value={branchId}
      ...
      {myStaff.length === 0 && !showForm ? (
        <div className="text-center py-16 rounded-2xl" style={{ background: 'var(--s)', border: '1px solid var(--bdl)' }}>
          <span className="material-symbols-rounded" style={{ fontSize: '64px', color: 'var(--t3)' }}>group</span>
          <p className="text-lg font-medium mb-2" style={{ color: 'var(--t2)' }}>No staff members yet</p>
          <p className="text-sm" style={{ color: 'var(--t3)' }}>Add your first staff member to get started</p>
        </div>
      ) : (
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--s)', border: '1px solid var(--bdl)', boxShadow: '0 1px 3px rgba(26,24,22,0.05)' }}>
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="min-w-full">
              <thead style={{ background: 'var(--bdl)' }}>
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--t)' }}>Staff Member</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--t)' }}>Phone Number</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--t)' }}>Branch</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--t)' }}>Status</th>
                </tr>
              </thead>
              <tbody>
                {myStaff.map((s, idx) => (
                  <tr key={s.id} className="transition-colors" style={{ borderBottom: idx < myStaff.length - 1 ? '1px solid var(--bdl)' : 'none' }}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium" style={{ color: 'var(--t)' }}>{s.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-mono text-sm" style={{ color: 'var(--t2)' }}>{s.phone}</td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium" style={{ background: 'var(--bdl)', color: 'var(--a)', border: '1px solid var(--bd)' }}>
                    ...
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium" style={{ background: 'var(--grbg)', color: 'var(--gr)', border: '1px solid rgba(42,96,64,0.2)' }}>
          ...
          <div className="md:hidden">
            {myStaff.map((s, idx) => (
              <div key={s.id} className="p-4 transition-colors" style={{ borderBottom: idx < myStaff.length - 1 ? '1px solid var(--bdl)' : 'none' }}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-base mb-1" style={{ color: 'var(--t)' }}>{s.name}</p>
                    <p className="text-sm font-mono mb-3" style={{ color: 'var(--t2)' }}>{s.phone}</p>
          ...
          {/* Footer with count */}
          <div className="px-6 py-4" style={{ background: 'var(--bdl)', borderTop: '1px solid var(--bd)' }}>
            <p className="text-sm" style={{ color: 'var(--t2)' }}>
              Total Staff: <span className="font-semibold" style={{ color: 'var(--t)' }}>{myStaff.length}</span>
            </p>
          </div>
        </div>
      )}
