...
import { activityApi, branchesApi } from '../../lib/api';
...
export function SellerDashboard() {
  ...
  const fetchList = useCallback(() => {
    if (!branchId) return;
    branchesApi
      .getActivities(branchId)
      .then((list) => setActivities(list.filter((a) => a.status === 'PENDING')))
  ...
  return (
    <div className="min-w-0 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 pb-20 sm:pb-24" style={{ background: 'var(--bg)' }}>
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2" style={{ color: 'var(--t)' }}>Pending Check-ins</h1>
        <p style={{ color: 'var(--t2)' }}>Approve, edit amount, or reject customer check-ins</p>
      </div>
      ...
      {activities.length === 0 ? (
        <div className="text-center py-16 rounded-2xl" style={{ background: 'var(--s)', border: '1px solid var(--bdl)' }}>
          <div className="text-6xl mb-4">
            <span className="material-symbols-rounded" style={{ fontSize: '64px', color: 'var(--gr)' }}>check_circle</span>
          </div>
          <p className="text-lg font-medium mb-2" style={{ color: 'var(--t2)' }}>All caught up!</p>
          <p className="text-sm" style={{ color: 'var(--t3)' }}>No pending check-ins at the moment</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:gap-5 lg:gap-6">
          {activities.map((a) => {
            ...
            return (
              <div key={a.id} className="rounded-2xl overflow-hidden transition-shadow hover:shadow-md" style={{ background: 'var(--s)', border: '1px solid var(--bdl)', boxShadow: '0 1px 3px rgba(26,24,22,0.05)' }}>
                <div className="p-4 sm:p-6">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-lg truncate" style={{ color: 'var(--t)' }}>
                        {a.customerName || a.customerId}
                      </h3>
                      {a.customerName && (
                        <p className="text-sm font-mono" style={{ color: 'var(--t2)' }}>{a.customerId}</p>
                      )}
                  ...
                  {requestedAmount != null && (
                    <div className="mb-4 p-3 rounded-lg" style={{ background: 'var(--bdl)', border: '1px solid var(--bd)' }}>
                  ...
                  <div className="mb-4">
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--t)' }}>
                      Transaction Amount
                    </label>
                    <input
                    ...
                    />
                    <p className="text-xs mt-1" style={{ color: 'var(--t3)' }}>Edit to override the requested amount</p>
                  </div>
      ...
      {rejectConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Confirm reject">
          ...
          <div className="relative rounded-2xl shadow-2xl p-6 max-w-md w-full" style={{ background: 'var(--s)', border: '1px solid var(--bdl)' }}>
            <div className="flex items-start gap-4 mb-4">
              <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: 'var(--rebg)' }}>
                <span className="material-symbols-rounded" style={{ color: 'var(--re)', fontSize: '24px' }}>warning</span>
              </div>
              <div>
                <h3 className="text-lg font-bold" style={{ color: 'var(--t)' }}>Reject Check-in?</h3>
                <p className="text-sm mt-1" style={{ color: 'var(--t2)' }}>The customer will not get this visit approved. This action cannot be undone.</p>
              </div>
            </div>
